                    ...
                    if (canx == 1) {
                        ...
                        if (canx == 1) {
                            for (let i = 0; i < mySprite.width; i++) {
                                if (TileCollisionArrayX.indexOf((Math.round((i + mySprite.left) * 16)) / 16) != -1) {
                                    for (let j = 0; j < mySprite.height; j++) {
                                        if (mySprite.image.getPixel(i,j) > 0) {
                                            if (TileCollisionArrayX.indexOf((Math.round((i + mySprite.left) * 16)) / 16) != -1 && TileCollisionArrayY.indexOf((Math.round((j + mySprite.top) * 16)) / 16) != -1) {
                                                mySprite.x += 0 - (curx - MiniTilemaps.readDataNumber(mySprite, "prevx")) / repeat
                                                canx = 0
                                            }
                                        }
