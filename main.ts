                    ...
                    if (canx == 1) {
                        ...
                        if (canx == 1) {
                            for (let i = 0; i < mySprite.width; i++) {
                                if (TileCollisionArrayX.indexOf(Math.trunc(i + mySprite.left)) != -1) {
                                    for (let j = 0; j < mySprite.height; j++) {
                                        if (mySprite.image.getPixel(i,j) > 0) {
                                            if (TileCollisionArrayX.indexOf(Math.trunc(i + mySprite.left)) != -1 && TileCollisionArrayY.indexOf(Math.trunc(j + mySprite.top)) != -1) {
                                                mySprite.x += 0 - (curx - MiniTilemaps.readDataNumber(mySprite, "prevx")) / repeat
                                                canx = 0
                                            }
                                        }
